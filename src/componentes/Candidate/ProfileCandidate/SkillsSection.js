...
const SkillsSection = () => {
  ...
  const handleDeleteSkill = (index) => {
    ...
    axios
      .delete(`${endpoints.profileDeleteSkill}/${skillToDelete.id}`)
      .then((response) => {
  ...
  const handleEditSkill = (index, newLevel) => {
    ...
    axios
      .put(`${endpoints.profileEditSkill}/${skillToEdit.id}`, skillToEdit)
      .then((response) => {
        setSkills(updatedSkills);
      })
      .catch((error) => {
  ...
  return (
    <div>
      ...
      <div className="mb-3">
        <label htmlFor="skillDropdown" className="form-label">
          Skills
        </label>
        <select
          id="skillDropdown"
      ...
      <button className="btn btn-primary mb-3" onClick={handleSaveSkills}>
        Guardar Skills
      </button>
            ...
            <tr key={myId()}>
              ...
              <td>
                <button
                  ...
                  onClick={() => handleEditSkill(index, "Nuevo Nivel")}
                >
                  Editar
                </button>
                <button
      ...
      <button className="btn btn-success" onClick={handleSendSkillsToAPI}>
        Enviar Skills a la API
      </button>
    </div>
  );
};
